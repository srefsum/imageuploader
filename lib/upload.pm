...
sub upload {
    ...
    if (defined($resp->{Params}{sub}) && $resp->{Params}{sub} ne '') {
        my $sub = $resp->{Params}{sub};
        unless ($sub =~ /^[A-Za-z0-9._-]+$/ && -d "$base_upload_path/$sub") {
            $resp->{variables}{result_type}    = 'error';
            $resp->{variables}{result_message} = 'Invalid or unknown subdirectory.';
            return;
        }
        $final_output_path = $base_upload_path . '/' . $sub . '/' . $safe_filename;
    } else {
    ...
    my $dest_label  = $description;
    if (defined($resp->{Params}{sub}) && $resp->{Params}{sub} ne '') {
        $dest_label .= ' / ' . $resp->{Params}{sub};
        $resp->{variables}{sub_key} = $resp->{Params}{sub};
    }
    $resp->{variables}{result_type}    = 'success';
    $resp->{variables}{result_message} = "File <strong>$safe_filename</strong> was uploaded to <strong>$dest_label</strong>.";
